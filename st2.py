...
def quantize_colors(frame: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        palette: Tensor (K, 3) com valores entre 0–1 ou -1–1

    Returns:
        frame_quant: Tensor (B, 3, H, W)
    """
    ...
    # (B, H, W, 3)
    flat = frame.permute(0, 2, 3, 1).reshape(-1, 3)  # (B*H*W, 3)
    dists = torch.cdist(flat, palette.to(frame.device))  # (B*H*W, K)

    indices = torch.argmin(dists, dim=1)  # (B*H*W,)
    quantized_flat = palette[indices]      # (B*H*W, 3)

    # Reconstrói o frame
    frame_quant = quantized_flat.view(B, H, W, 3).permute(0, 3, 1, 2)
    return frame_quant
...
class VideoDecoderNoSpatial(nn.Module):#ultima
    ...
    def forward(
        ...
        if teacher_forcing_frames is not None:
            assert teacher_forcing_frames.shape[2] >= T
...
class ST2(nn.Module):
    ...
    def forward_autoregressive(
        ...
        for t in range(T_in):
            ...
            quant_frame = quantize_colors(pred_frame)  
            pred_frame = quant_frame
